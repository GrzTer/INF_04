...
class Film:
    ...
    def set_title(self, new_title: str) -> None:
        if len(new_title) <= film.MAX_LENGHT_TITLE:
            self.__title = new_title
        else:
            return "Błąd: Tytuł może mieć maksymalnie 20 znaków, a twój ma ich: " + str(
                len(new_title)
            )
    ...
    def get_num_of_rental(self) -> int:
        return self.__num_of_rental

    def increment(self) -> None:
        self.__num_of_rental += 1
...
def main():
    ...
    print(
        f"Test ograniczenia długości tytulu{Film.set_title("Bardzo długi tytuł, który sprawdza działanie ograniczenia")}"
    )
    print(f"Tytuł: {Film.get_title()}\n")
