...
def pseudolos_lista(size: int = 50, low: int = 1, high = 100) -> list[int]: return [random.randint(low, high) for _ in range(size)]
...
def szukanie_wartownik(arr: list[int], x: int) -> int:
    ...
    i = 0
    while arr[i] != x: i += 1 # Pętla sprawdzająca pierwsze wystąpienie wartownika `x`
...
def main() -> None:
    ...
    try: x = int(input("Podaj liczbę, której mam szukać (1-100): "))
    except ValueError:
